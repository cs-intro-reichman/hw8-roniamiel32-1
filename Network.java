    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for(int i = 0; i < userCount; i++ ){
            ...
            if (currentUser == user || user.follows(currentName)) continue;
            int mutual = user.countMutual(currentUser);
            if(mutual > max){
                max = mutual;
                recommendedUser = currentName;
           }
        }
    ...
    public String mostPopularUser() {
        ...
        int max = followeeCount(mostPopular.getName());
        for(int i = 1; i < userCount; i++ ){
            String userName = users[i].getName();
            if (followeeCount(userName) > max){
                max = followeeCount(userName);
                mostPopular = users[i];
            }
        }
